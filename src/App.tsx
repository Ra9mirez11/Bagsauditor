...
const App = () => {
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'ai', content: `Error: ${e.message}. Please check your OpenRouter API key.` }]);
    } finally {
      setIsSending(false);
    }
  };
    ...
    const runAudit = async (forcedMint?: string) => {
      ...
      try {
        ...
        setChatMessages(prev => [...prev, { role: 'ai', content: `Security audit failed: ${error.message}. Please verify your API configuration.` }]);
      } finally {
        setIsAuditing(false);
